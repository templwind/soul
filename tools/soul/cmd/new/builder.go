...
//go:embed templates/** templates/.github/**
...
// WithClientConfigs sets the client configurations for the builder
func WithClientConfigs(configs []ClientConfig) optFunc[SaaSBuilder] {
	return func(sb *SaaSBuilder) {
		sb.ClientConfigs = configs
	}
}
...
func (sb *SaaSBuilder) genFile(c fileGenConfig) error {
	...
	text, err := fs.ReadFile(sb.TemplatesFS, c.templateFile)
	if err != nil {
...
func (sb *SaaSBuilder) processFiles() error {
	...
	err := fs.WalkDir(sb.TemplatesFS, "templates", func(path string, d fs.DirEntry, err error) error {
		if err != nil || d.IsDir() {
			return err
		}
