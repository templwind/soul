<script lang="ts">
	...
	async function handleSubmit(event: SubmitEvent) {
		...
		try {
			const response = await api.RegisterPost({
...
<div class="z-10 flex flex-col flex-1 px-4 py-10 bg-base-100 md:flex-none md:px-28">
	<main class="w-full max-w-md mx-auto sm:px-4 md:w-96 md:max-w-sm md:px-0">
		...
		<h2 class="mt-20 text-lg font-semibold text-base-content">Get started for free</h2>
		...
		<form on:submit={handleSubmit} class="grid grid-cols-1 mt-10 gap-y-4">
			...
			<div class="grid grid-cols-1 gap-y-4">
				<div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
					<label for="firstName" class="w-full form-control">
					...
					<label for="lastName" class="w-full form-control">
		...
		<div class="mt-8">
			<p class="text-sm text-base-content/70">
				By clicking the button above, you agree to our <a href="/terms" target="_blank" class="link"
					>terms of service</a
				>
		...
		<p class="mt-8 text-sm text-center text-base-content/70">
			Already registered?
			<a href="/auth/login" class="font-medium link link-primary">Sign in</a> to your account.
		</p>
	</main>
</div>
