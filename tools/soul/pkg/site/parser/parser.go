		...
		if state == "requestObject" {
			...
			if strings.Contains(part, "(") {
				if strings.Contains(part, "()") || strings.EqualFold(part, "returns") {
					...
					method.RequestType = p.parseType(method.Request) // Use parseType here
					state = "responseObject"
				}
			}
			continue
...
func (p *Parser) parseType(typeStr string) interface{} {
	...
	if strings.HasPrefix(typeStr, "*") {
		// Remove pointer notation and create pointer type
		baseTypeStr := strings.TrimPrefix(typeStr, "*")
		return spec.NewPointerType(
			spec.NewStructType(strings.TrimSpace(baseTypeStr), nil, nil, nil),
		)
	}

	// Check for array type
	if strings.Contains(typeStr, "[]") {
		// Remove array notation and create array type
		baseTypeStr := strings.Replace(typeStr, "[]", "", -1)
		return spec.NewArrayType(
			spec.NewStructType(strings.TrimSpace(baseTypeStr), nil, nil, nil),
		)
	}

	// Return regular struct type
	return spec.NewStructType(strings.TrimSpace(typeStr), nil, nil, nil)
}
...
func (p *Parser) parseBidirectionalString(input string) (requestTopic string, requestType interface{}, responseTopic string, responseType interface{}, err error) {
	...
	// Use the parseType function for both request and response types
	requestType = p.parseType(requestTypeStr)
	responseType = p.parseType(responseTypeStr)
...
func (p *Parser) parsePubSubString(input string) (subscriptionTopic string, requestType interface{}, publishTopic string, responseType interface{}, err error) {
	...
	// Use parseType for both request and response types
	requestType = p.parseType(requestTypeStr)
	responseType = p.parseType(responseTypeStr)
...
func (p *Parser) parseSubTopic(method *ast.MethodNode) {
	...
	// Check if the topic has an ending publication
	if strings.Contains(literal, " pub ") {
		// Parse pub/sub pattern with response
		requestTopic, requestType, responseTopic, responseType, err := p.parsePubSubString(literal)
		if err != nil {
			fmt.Println("ERROR: Cannot parse pubsub method", err)
		}
		...
		// Handle simple subscription without publication
		parts := strings.SplitN(literal, " ", 3)
		if len(parts) != 2 {
			fmt.Println("ERROR: Invalid sub topic format")
			p.nextToken()
		...
		// Use parseType for the request type
		requestType := p.parseType(requestTypeStr)
...
func (p *Parser) parseSocketMethod(method *ast.MethodNode) {
	...
	for p.curToken.Type != lexer.CLOSE_PAREN {
		...
		if strings.Contains(literal, "<<>>") {
			...
			isClientInitiated := strings.Contains(literal, ">>")
			// Replace the >> and << with section separator
			literal = strings.ReplaceAll(literal, "<<", "\u00A7")
			literal = strings.ReplaceAll(literal, ">>", "\u00A7")
			...
			if len(requestParts) > 1 {
				typeStr := strings.Trim(requestParts[1], "()")
				requestType = p.parseType(typeStr)
			}
			...
			if len(sections) > 1 {
				responseParts := strings.Split(strings.TrimSpace(sections[1]), " ")
				if len(responseParts) > 0 {
					typeStr := strings.Trim(responseParts[0], "()")
					responseType = p.parseType(typeStr)
				}
			}
